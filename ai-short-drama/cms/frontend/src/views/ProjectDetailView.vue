...
const isVersionedAdaptationProject = computed(() => (
  project.value?.config?.contract_version === '2.0' && Boolean(project.value?.config?.source_version_id)
))
const requiresAdaptationPlan = computed(() => isVersionedAdaptationProject.value && !isRollingProduction.value)
...
const productionTabs = computed(() => {
  ...
  return [
    { key: 'workflow_tasks', label: '工作流任务', icon: GitBranch, items: project.value.workflow_tasks, columns: [
      ...
      { key: 'retry_action', label: '操作', type: 'action', visible: (item) => item.status === 'failed' && !requiresAdaptationPlan.value, disabled: (item) => flowLoading.value || activeWorkflowTasks.value > 0 || retryingTaskId.value === item.task_id, labelFor: (item) => retryingTaskId.value === item.task_id ? '重试中…' : '重试' },
    ] },
