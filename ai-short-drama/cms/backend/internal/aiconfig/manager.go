...
var FieldSpecs = []FieldSpec{
	...
	{Key: "TTS_PROVIDER", Label: "语音供应商", Category: "语音合成", Kind: "select", Options: []string{"mock", "google_gemini_speech", "google_vertex_gemini_speech", "google_chirp3_hd", "generic_sync_tts", "generic_async_tts"}},
	{Key: "TTS_MODEL", Label: "语音模型", Category: "语音合成", Kind: "suggest", Options: []string{"gemini-3.1-flash-tts-preview", "gemini-2.5-flash-preview-tts", "gemini-2.5-pro-preview-tts", "gemini-2.5-flash-tts", "gemini-2.5-pro-tts", "gemini-2.5-flash-lite-preview-tts", "chirp-3-hd", "mock-tts"}, Description: "Gemini Developer API 与 Vertex AI 使用各自可用的 Gemini TTS 模型 ID；Chirp 3 HD 使用 chirp-3-hd。"},
	{Key: "TTS_API_BASE_URL", Label: "语音 API 地址", Category: "语音合成", Kind: "url"},
	{Key: "TTS_VERTEX_PROJECT_ID", Label: "Vertex AI Project ID", Category: "Google Vertex AI 语音", Kind: "text", Description: "留空时从共用的 Google 服务账号 JSON 自动读取。", AllowEmpty: true, Target: "video-adapter"},
	{Key: "TTS_VERTEX_LOCATION", Label: "Vertex AI TTS 区域", Category: "Google Vertex AI 语音", Kind: "select", Options: []string{"global", "us-central1", "us-east1", "us-east4", "us-east5", "us-south1", "us-west1", "us-west4", "europe-central2", "europe-north1", "europe-southwest1", "europe-west1", "europe-west4", "northamerica-northeast1"}, Description: "Gemini 3.1 Flash TTS 使用 global；2.5 系列还支持列出的区域。", Target: "video-adapter"},
...
var SecretSpecs = []SecretSpec{
	...
	{Key: "VEO_SERVICE_ACCOUNT_JSON", Label: "Google Vertex AI 服务账号 JSON", Kind: "service_account_json", Description: "Veo 与 Vertex AI TTS 共用；系统只保存完整服务账号 JSON，不回显。", Target: "video-adapter"},
