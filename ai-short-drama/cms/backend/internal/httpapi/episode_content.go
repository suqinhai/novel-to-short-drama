...
func (h *Handler) updateEpisodeRunContent(c *gin.Context) {
	...
	result, err := h.store.UpdateEpisodeContent(
		c.Request.Context(), c.Param("projectID"), c.Param("episodeRunID"), input,
	)
	switch {
	case errors.Is(err, store.ErrNotFound):
		respondError(c, http.StatusNotFound, "EPISODE_CONTENT_NOT_FOUND", "单集内容不存在")
	...
	case err != nil:
		respondError(c, http.StatusInternalServerError, "EPISODE_CONTENT_UPDATE_FAILED", "单集内容保存失败："+err.Error())
	default:
		c.JSON(http.StatusOK, gin.H{"data": result})
	}
}
