...
assert.match(load, /COALESCE\(t\.status,''\) NOT IN \('pending','submitting','processing','succeeded'\)/);
assert.match(load, /NOT \(\s*COALESCE\(v\.status,''\)='succeeded'\s*AND COALESCE\(v\.auto_qc_status,''\) IN \('passed','warning'\)\s*\)/);
assert.match(load, /AND \(\s*\$4::text IS NOT NULL[\s\S]*?\)\s*ORDER BY sh\.shot_order LIMIT \$5::int/);
assert.match(prepare, /alreadySucceeded=s\.current_video_id&&s\.current_video_status==='succeeded'&&\['passed','warning'\]\.includes/);
...
assert.doesNotMatch(state, /ORDER BY sh\.shot_order LIMIT \$5::int/);
...
assert.match(workflow.nodes.find((node) => node.name === 'Build Video Dispatch Result').parameters.jsCode, /const ok=required>0&&failed===0;/);
