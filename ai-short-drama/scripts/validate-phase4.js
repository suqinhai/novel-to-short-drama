...
assert(/\(SELECT failed_count>0 FROM stats\)\s*audio_failed/.test(voiceAudioWorkflow) && voiceAudioWorkflow.includes("'TTS_TASKS_FAILED'"), '10-voice-audio.json: failed dialogue audio must surface stage_4_failed instead of audio_processing');
