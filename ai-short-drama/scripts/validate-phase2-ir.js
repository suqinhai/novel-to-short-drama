...
if (extract) {
  for (const name of ['Claim IR Operation', 'Claim Already Ready for Reconcile?', 'Load One Bounded Chapter Window', 'Checkpoint Model Call Started', 'Call Narrative Extraction Model', 'Normalize Model Entity Shape', 'Normalize Model Numeric Shape', 'Normalize Model Provenance and Require Events', 'JSON Schema Validate', 'Business Validate Provenance and References', 'Canonicalize Validated Window Hash', 'Checkpoint Validated Window', 'Sanitize IR Extraction Failure', 'Atomically Quarantine Failed IR Run']) requireNode(extract, name);
  if (!connected(extract, 'Verify Claimed Operation', 'Claim Already Ready for Reconcile?') || !connected(extract, 'Claim Already Ready for Reconcile?', 'Execute 02b Narrative IR Reconcile') || !connected(extract, 'Claim Already Ready for Reconcile?', 'Load One Bounded Chapter Window')) fail('02a: claimed ready checkpoints must resume directly at reconcile without another model call');
  if (!connected(extract, 'Parse Model JSON', 'Normalize Model Entity Shape') || !connected(extract, 'Normalize Model Entity Shape', 'Normalize Model Numeric Shape') || !connected(extract, 'Normalize Model Numeric Shape', 'Normalize Model Provenance and Require Events') || !connected(extract, 'Normalize Model Provenance and Require Events', 'JSON Schema Validate') || !connected(extract, 'JSON Schema Validate', 'Business Validate Provenance and References') || !connected(extract, 'Business Validate Provenance and References', 'Canonicalize Validated Window Hash') || !connected(extract, 'Canonicalize Validated Window Hash', 'Checkpoint Validated Window')) fail('02a: model shape/numeric/provenance normalization, schema, business and canonical hash validation must precede checkpointing');
  for (const source of ['Verify Claimed Operation', 'Load One Bounded Chapter Window', 'Build Codepoint-bounded Slice', 'Checkpoint Model Call Started', 'Call Narrative Extraction Model', 'Parse Model JSON', 'Normalize Model Entity Shape', 'Normalize Model Numeric Shape', 'Normalize Model Provenance and Require Events', 'JSON Schema Validate', 'Business Validate Provenance and References', 'Canonicalize Validated Window Hash', 'Checkpoint Validated Window', 'Checkpoint Ready Without More Windows', 'Execute 02b Narrative IR Reconcile']) {
...
if (reconcile) {
  for (const name of ['Assert Fenced Claim and Load Windows', 'Canonicalize Checkpoint Windows', 'Revalidate All Windows', 'Deterministic Cross-window Reconcile', 'Normalize Publish Document Defaults', 'Require Non-empty Narrative IR', 'Atomic Publish Narrative IR', 'Published Result']) requireNode(reconcile, name);
  ...
  if (!connected(reconcile, 'Deterministic Cross-window Reconcile', 'Normalize Publish Document Defaults') || !connected(reconcile, 'Normalize Publish Document Defaults', 'Require Non-empty Narrative IR') || !connected(reconcile, 'Normalize Publish Document Defaults', 'Sanitize Quarantine Failure') || !connected(reconcile, 'Require Non-empty Narrative IR', 'Atomic Publish Narrative IR') || !connected(reconcile, 'Require Non-empty Narrative IR', 'Sanitize Quarantine Failure')) fail('02b: publish defaults and non-empty IR guard must run before atomic publication and quarantine failures');
