...
视频默认 `VIDEO_PROVIDER=mock`、`VIDEO_MODEL=mock-image-to-video`。CMS 提供独立的 Google 视频配置区，可在 `gemini-omni-flash-preview`（Gemini Omni）、`veo-3.1-generate-001`（Veo 3.1）和 `veo-3.1-fast-generate-001`（Veo 3.1 Fast）之间点击切换，并安全填写服务账号 JSON、Project ID、Veo 区域及 GCS 目录。仓库内置的异步 Google 视频适配器会按模型自动选择 Interactions API 或 Veo 长任务协议，部署与最小权限配置见 [`docs/vertex-veo-adapter.md`](docs/vertex-veo-adapter.md)。TTS 默认 `TTS_PROVIDER=mock`、`TTS_MODEL=mock-tts`；除 `generic_sync_tts` 和 `generic_async_tts` 外，10a 已原生支持 `google_gemini_speech`（Gemini Developer API）、`google_vertex_gemini_speech`（Vertex AI Gemini TTS）和 `google_chirp3_hd`（Chirp 3 HD）。Vertex TTS 与 Google 视频适配器共用安全服务账号和短期 OAuth Token，CMS 可一键填写路由、推荐模型、区域及默认声线，部署和声线格式见 [`docs/google-speech-adapters.md`](docs/google-speech-adapters.md)。真实 Key 只放本地托管配置或运行时密钥，不得写入工作流 JSON、test-data、数据库 request/response payload 或日志。
